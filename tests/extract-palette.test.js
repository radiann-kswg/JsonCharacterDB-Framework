...
const SAMPLE_PNG = path.join(
    REPO_ROOT, 'data', 'Works_Sample', 'Images', 'DB_Primary',
    'concept', 'alpha.png',
);
...
describe('resolveImageSources — 画像ソースの優先順（$palette.source 駆動）', () => {
    const NTS_WORK_DIR = path.join(REPO_ROOT, 'data', 'Works_Sample');
    ...
    it('illustration → artwork → swatch の順に返す', () => {
        const sources = resolveImageSources(
            {
                Images: {
                    concept_PNGName: 'alpha',
                    corefolder_PNGPath: ['alpha'],
                    arts_PNGPath: ['alpha-1'],
                },
            },
            NTS_WORK_DIR, NTS_IMAGES,
        );
