...
describe('catalog meta schema declarations', () => {
  ...
  it('work-local db_meta databases can expose DB labels', () => {
    const workMeta = load('data/Works_Sample/DataBases/db_meta.json');
    const refMeta = load('data/Works_Sample/References/db_meta.json');
  ...
  it('work-local db_meta story era values match the structured schema fields', () => {
    ...
    expect(primary?.StoryEra?.FromEra?.[0]).toMatchObject({ EraGen: 1, YearInEra: 1 });
    expect(primary?.StoryEra?.InEra?.[2]).toMatchObject({ byRealYear: 2020 });
