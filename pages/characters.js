...
async function fetchGlobalDefType() {
	...
	const isValid = (obj) => {
		...
		//   2') ただしグローバル `data/Dictionaries/` を持たない構成（辞書が作品スコープだけのリポジトリ）では
		//       #Dict_* が 1 つも生えない。この場合は `$DefType` の有無で代替判定する。
		//       `$DefType` は deftype エンドポイントの応答にだけ含まれ、素の db_meta.json 単体には無いため、
		//       「古いキャッシュを掴んだ」ケースを取りこぼさずに判別できる。
		...
		if (Object.keys(vars).some((k) => k.startsWith('#Dict_'))) return true;
		return Array.isArray(obj.$DefType) && obj.$DefType.length > 0;
	};
	...
	const imageResolvers = [
		...
		() => {
			if (img.conceptAlt_PNGName) {
				...
				// 空配列・空文字は「画像なし」。ここで弾かないと `undefined.png` を参照してしまう
...
export async function renderDetail(workId, rec) {
	...
	try {
		...
		const shownKeys = (() => {
			...
			// ヒーロー帯の pill に出したキーは基本情報テーブルへ二重表示しない。
			// 対象は作品側の `$DetailLayout.headerPills` 宣言に従う（Progress 決め打ちにしない）。
			for (const pillKey of (Array.isArray(detailLayout?.headerPills) ? detailLayout.headerPills : ['Progress'])) {
				const key = String(pillKey ?? '').trim();
				if (!key) continue;
				const { usedKey } = getValueWithAlt(key);
				const resolved = usedKey || key;
				if (rec?.[resolved] != null && rec[resolved] !== '') s.add(resolved);
			}
