...
/**
 * 旧綴りの作品IDを現行綴りへ寄せる別名表（公開済み直リンクの読み取り互換専用。生成側は常に現行綴り） */
const LEGACY_WORK_ALIASES = { ShouArRiders: 'ShauErRiders' };
...
export function workKeyForURL(workKey) {
	...
	if (!raw) return '';
	const short = raw.replace(/^#/, '').replace(/^Works_/, '');
	return LEGACY_WORK_ALIASES[short] || short;
}
...
export function parseViewerLocator(raw) {
	...
	if (firstSlash < 0) return { ...empty, work: workKeyForURL(value) };

	const work = workKeyForURL(value.slice(0, firstSlash));
