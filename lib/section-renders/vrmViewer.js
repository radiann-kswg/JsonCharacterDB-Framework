...
(() => {
	...
	async function initViewer({ stage, vrmUrl }) {
		...
		if (vrm) {
			...
			scene.add(vrm.scene);
			// 固定カメラ位置は等身の低いモデルで下に沈むため、実寸のバウンディングボックス中心へフィットさせる
			const box = new THREE.Box3().setFromObject(vrm.scene);
			const center = box.getCenter(new THREE.Vector3());
			const size = box.getSize(new THREE.Vector3());
			const fov = THREE.MathUtils.degToRad(camera.fov);
			const dist = (Math.max(size.y, size.x / camera.aspect) / 2) / Math.tan(fov / 2) * 1.4;
	...
	function buildViewerCard(relPath, folderHint, helpers, lang) {
		...
		const poster = thumbUrl
			? el('img', {
				...
				loading: 'lazy',
				// サムネイル未登録（404）時は壊れた画像を出さず、ビューアのみのカードへフォールバック
				onerror: (ev) => ev.currentTarget.closest('.model-viewer__media')?.remove()
			}, [])
			: null;
