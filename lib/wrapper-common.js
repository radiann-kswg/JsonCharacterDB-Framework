...
(function initializeCharacterValueWrapperRegistry(root) {
	...
	function formatDaySummary(value, context) {
		...
		// `year` ロールは任意。誕生日のように年を持たない `$Def_Day` 宣言では空のままになる。
		const rawYear = pickRoleRawValue(dayValue, typeSources, '$Def_Day', 'year') ?? dayValue.Year;
		const year = rawYear != null ? String(rawYear).trim() : '';

		const jpMonthDay = (month && dayOfMonth) ? `${month}月${dayOfMonth}日` : (month || dayOfMonth);
		const enMonthDay = (monthEn && dayOfMonth) ? `${monthEn}.${dayOfMonth}` : (monthEn || dayOfMonth);
		const jpDate = year ? (jpMonthDay ? `${year}年${jpMonthDay}` : `${year}年`) : jpMonthDay;
		const enDate = year ? (enMonthDay ? `${enMonthDay}, ${year}` : year) : enMonthDay;
