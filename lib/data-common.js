...
const DATA_COMMON_LEGACY_WORK_DIR_ALIASES = { Proxies: 'Works_DestinyFoxRecords', ShouArRiders: 'Works_ShauErRiders' };

// 旧綴りの作品ID → 現行綴り（獣爾騎兵: ShouArRiders → ShauErRiders）。
// normalizeWorkId() / toWorkKeyFromWorksTitle() の正規化時点で読み替え、db_meta / D1 の現行キーへ揃える。
// NOTE: sw-common.js の `LEGACY_WORK_ID_ALIASES` と同一グローバルへ読み込まれるため別名で保持する（上記と同じ理由）。
const DATA_COMMON_LEGACY_WORK_ID_ALIASES = { ShouArRiders: 'ShauErRiders' };

/**
 * 作品IDを `#Works_<Name>` 形式へ正規化し、旧綴りの作品IDは現行綴りへ読み替える
 * @param {string} workId - 'NumberTales' / 'Works_NumberTales' / '#Works_NumberTales'
 * @returns {string} '#Works_NumberTales'（空入力は空文字）
 */
function normalizeLegacyWorkKey(workId) {
	const s = String(workId || '').trim();
	if (!s) return '';
	let normalized;
	if (s.startsWith('#Works_')) normalized = s;
	else if (s.startsWith('Works_')) normalized = `#${s}`;
	else normalized = `#Works_${s}`;
	const bare = normalized.slice('#Works_'.length);
	return `#Works_${DATA_COMMON_LEGACY_WORK_ID_ALIASES[bare] || bare}`;
}
...
class ReferenceResolver {
	...
	normalizeWorkId(workId) {
		// 旧綴り互換（ShouArRiders → ShauErRiders）を含む
		return normalizeLegacyWorkKey(workId);
	}
...
class EnrichmentProcessor {
	...
	toWorkKeyFromWorksTitle(worksTitle) {
		// 旧綴り互換（ShouArRiders → ShauErRiders）を含む
		return normalizeLegacyWorkKey(worksTitle);
	}
	...
	buildDisplaySections(record, defTypeMerged, indices = null) {
		...
		const sectionOrder = ['basic', 'profile', 'spec', 'sub', 'images', 'other'];
...
class TypeDefUtils {
	 ...
	 * @param {Object} entry
	 * @returns {'basic'|'profile'|'spec'|'sub'|'images'|'other'}
	 */
	static pickDisplaySection(entry) {
		...
		if (explicit && typeof explicit === 'string') {
			...
			if (s === 'basic' || s === 'profile' || s === 'spec' || s === 'sub' || s === 'images' || s === 'other') return s;
		}
...
class ImageProcessor {
	 ...
	 *   ここへ渡る `dbName` は SW ルート（`/pages/v1/<work>/<db>`）由来の**接頭辞なしの素名**のため、
	 *   `#DB_*` は既定の `DB_` 補完で足りるが、`#Ref_*` / `#Loc_*` は素名から判別できない。
	 *
	 *   **素名だけでは原理的に一意に決まらない**（実データに `#Ref_Society` と `#Loc_Society` が併存する）。
	 *   正しく解くには DB カタログの `DB_Layer` が要るが、本メソッドの呼び出し経路
	 *   （`enrichRecords` → `imageFromRecord` → `processImageValue` → `resolveImagePath`）は
	 *   レイヤー情報を持たない。UI 側（`pages/characters.js`）は `DB_Layer` を引数で受け取れるため
	 *   汎用分岐で解けており、そちらが本来の姿。
	 *
	 *   そこで本メソッドは「**画像を実際に持つ資料系 DB だけ**」を例外表として持つ。
	 *   実データ上、画像を持つ `#Ref_*` / `#Loc_*` は `Vocabulary` / `Reference` の 2 つだけで、
	 *   他の 4 Ref + 16 Loc は画像を 1 枚も持たない（`tests/image-dir-mapping.test.js` が実データで固定）。
	 *
	 * @param {string} dbName - データベース名（接頭辞なしの素名、または `DB_`/`Ref_`/`Loc_` 付き）
	...
	mapDbNameToImageDir(dbName) {
		...
		// 画像を持つ資料系 DB のみ。`Glossary` は `#Ref_Vocabulary` へ改名済みで実体が無い
		// （2026-09-07 に追従。それまで References 画像が `DB_Vocabulary` へ誤解決していた）
		const refMapping = {
